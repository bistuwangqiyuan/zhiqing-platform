...
import { getPost, getPosts, POST_SLUGS } from "@/lib/content/posts";
...
export async function generateMetadata({ params }: Params) {
  const p = getPost(params.locale, params.slug);
  if (!p) return { title: params.locale === "en" ? "Insights" : "洞察" };
  return {
    title: p.title,
        ...
        <div className="container max-w-3xl">
          ...
          <Reveal>
            <div className="mt-6 flex items-center gap-2 flex-wrap">
              {p.tags.map((tag) => (
                <span key={tag} className="text-[11px] px-2.5 py-1 rounded-full bg-ink-50 text-ink-500 border border-ink-100">
                  {tag}
                </span>
              ))}
            </div>
            ...
            <div className="mt-5 flex items-center gap-5 text-[13px] text-ink-500">
              ...
              <span className="inline-flex items-center gap-1.5"><Clock className="h-4 w-4" />{t("readMin", { min: readMin })}</span>
            </div>
          </Reveal>
        </div>
      </section>
