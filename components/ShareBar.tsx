...
export function ShareBar({ title, className = "" }: { title: string; className?: string }) {
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <span className="text-[12px] text-ink-500">{t("label")}</span>
      ...
      <a href={wb} target="_blank" rel="noreferrer" className="h-9 w-9 rounded-full grid place-items-center bg-ink-50 hover:bg-ink-100 transition-colors text-ink-700" aria-label={t("weibo")}>
      ...
      >
        {copied ? <><Check className="h-3.5 w-3.5 text-emerald-600" /> {t("copied")}</> : <><LinkIcon className="h-3.5 w-3.5" /> {t("copy")}</>}
      </button>
    </div>
  );
}
